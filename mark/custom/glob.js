...
var handlers = {
	...
	onIndexUnload : function(){
		window.DummyCookie && window.DummyCookie.flush(); // 更新 Cookie 值
		return true;
	}
};
...
function setRowColorClassTables(selectorText,doc)
{
    ...
    if (table){
        var show = DummyCookie.get("showInherited" + selectorText) === 'true';
        var rowNum = 0;
        for (var i = 1,len = table.rows.length; i < len; i++){
            if(show){
                if(table.rows[i].style.display.indexOf('none') === -1){
...
function setFilterCookie(name, value){
	...
	DummyCookie.set(name.toLowerCase() , value.toLowerCase() , expire.getTime());
}
 ...
 * 由 header.html 中的 header.js 最先调用 onReady
